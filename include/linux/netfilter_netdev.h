...
static inline bool nf_hook_netdev_active(enum nf_dev_hooks hooknum,
					 struct nf_hook_entry __rcu *hooks)
{
...
static inline int nf_hook_netdev(struct sk_buff *skb, enum nf_dev_hooks hooknum,
				 struct nf_hook_entry __rcu *hooks)
{
	struct nf_hook_entry *e = rcu_dereference(hooks);
	...
	nf_hook_state_init(&state, e, hooknum, INT_MIN,
